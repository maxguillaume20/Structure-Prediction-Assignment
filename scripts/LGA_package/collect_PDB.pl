        ...
        if(/^MODEL /) {
          ...
          if($ATOMS{$atom} ne $atom) {
            if($RESNUMB{$residue_numb} eq $residue_numb && $residue_name ne $residue_name_prev) {
              if($r_insr eq ' ' && $a_altp eq ' ') { $r_insr='A'; $fixed=1; }
            }
            if($RESNUMB{$residue_numb} ne $residue_numb || $residue_name eq $residue_name_prev ||
               $fixed == 1) {
            ...
            if($print_ok==1 && $RESNUMB{$residue_numb} ne $residue_numb) {
